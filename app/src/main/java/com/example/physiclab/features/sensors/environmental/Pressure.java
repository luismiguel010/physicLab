...
public class Pressure extends AppCompatActivity implements OnChartValueSelectedListener, SensorEventListener {
    ...
    ArrayList<Float> vectorTime, vectorPressure;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.play:
                ...
                Toast.makeText(Pressure.this, "Sensor de presión activado.", Toast.LENGTH_LONG).show();
                return true;
            case R.id.stop:
                isSensorOn = false;
                pauseTime();
                ...
                Toast.makeText(Pressure.this, "Sensor de presión desactivado.", Toast.LENGTH_LONG).show();
                return true;
            case R.id.exportData:
                export();
                Toast.makeText(Pressure.this, "Exportar datos.", Toast.LENGTH_LONG).show();
                return true;
    ...
    private LineDataSet createSetC() {
        LineDataSet set = new LineDataSet(null, "Presión de aire ambiental en mbar");
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        if(isSensorOn) {
            ...
            viewPressure.setText(axisC + " mbar");
    ...
    private void saveData(float secsWithMillis, float axisC) {
        if(isSensorOn) {
            ...
            vectorPressure.add(axisC);
        }
    }

    public void export(){
        String nameFile = "DataPressure";
        StringBuilder data = new StringBuilder();
        data.append("Tiempo,mbar");
        for(int i = 0; i < vectorTime.size(); i++){
            data.append("\n" + vectorTime.get(i).toString() + "," + vectorPressure.get(i).toString());
        }
        try{
            FileOutputStream out = openFileOutput("pressure.csv", Context.MODE_PRIVATE);
            out.write((data.toString()).getBytes());
            out.close();
            Context context = getApplicationContext();
            File filelocation = new File(getFilesDir(), "pressure.csv");
